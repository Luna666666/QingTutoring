...
- (void)startRequestWithProgress:(void (^)(NSProgress *))progress success:(void (^)(id))success failure:(void (^)(NSError * error))failure {
    ...
    [manager.requestSerializer didChangeValueForKey:@"timeoutInterval"];
    NSString *URLSting = [self.url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
    NSLog(@"url:%@",URLSting);
    ...
    switch (self.wRequestType) {
        case GET: {
            [manager GET:URLSting parameters:self.parameters progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        case POST: {
            if (self.formData) {
                [manager POST:URLSting parameters:self.parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
                ...
                [manager POST:URLSting parameters:self.parameters progress:^(NSProgress * _Nonnull uploadProgress) {
                    if (progress) {
                        progress(uploadProgress);
                    }
                } success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        case PUT: {
            [manager PUT:URLSting parameters:self.parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
                success(responseObject);
        ...
        case PATCH: {
            [manager PATCH:URLSting parameters:self.parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
                success(responseObject);
            } failure:^(NSURLSessionDataTask * _Nonnull task, NSError * _Nonnull error) {
                failure(error);
            }];
        ...
        case DELETE: {
            [manager DELETE:URLSting parameters:self.parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
                success(responseObject);
            } failure:^(NSURLSessionDataTask * _Nonnull task, NSError * _Nonnull error) {
                failure(error);
            }];
