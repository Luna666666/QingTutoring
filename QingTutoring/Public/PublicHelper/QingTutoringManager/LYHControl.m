...
+ (void)DebugRequestWithPostUrlStr:(NSString *)urlStr andParam:(NSDictionary *)param {
    [[HttpManager shareManager] requestReturnStringWithPostUrlString:urlStr parameters:param successBlock:^(NSURLSessionDataTask *operation, id responseObject) {
        NSString *responseStr = [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
        DEBUGLOG(@"测试错误请求返回%@", responseStr);
    } failureBlock:^(NSURLSessionDataTask *operation, NSError *error) {
        DEBUGLOG(@"测试错误请求，返回错误%@", error);
    }];
}
